...
// Mapping of user IDs to encoded values
$encodingMap = [
    1 => '*&ABCD',
    2 => '*&BUYG',
    3 => ')(*YH',
...
function encodeId($id, $map) {
    return isset($map[$id]) ? $map[$id] : null; // Return encoded string
}

// Function to decode ID
function decodeId($encodedId, $map) {
    $flippedMap = array_flip($map); // Reverse the map for decoding
    return isset($flippedMap[$encodedId]) ? $flippedMap[$encodedId] : null; // Return original ID or null if not found
}

// Check if there's an ID in the URL
if (!empty($_GET['id'])) {
    $_id = decodeId($_GET['id'], $encodingMap); // Decode ID from URL
    if ($_id !== null) {
        $user = $userModel->findUserById($_id); // Find user to update
    }
}
// if (!empty($_GET['id'])) {
//     $_id = $_GET['id'];
